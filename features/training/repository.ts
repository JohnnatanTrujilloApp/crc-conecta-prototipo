...
export type LessonResource={id:string;lessonId:string;organizationId:string;title:string;type:ResourceType;sourceUrl:string;storagePath:string;mimeType:string;fileSize:number|null;sortOrder:number;embedProvider:string};
...
export async function loadTraining(){
 ...
 const [siteResult,programResult,moduleResult,lessonResult,resourceResult]=await Promise.all([
  ...
  client.from("lesson_resources").select("id,lesson_id,organization_id,title,resource_type,source_url,storage_path,mime_type,file_size_bytes,sort_order,embed_provider").eq("active",true).order("sort_order")
 ]);
 for(const result of [siteResult,programResult,moduleResult,lessonResult,resourceResult])if(result.error)throw result.error;
 const resources:LessonResource[]=(resourceResult.data??[]).map(row=>({id:row.id,lessonId:row.lesson_id,organizationId:row.organization_id,title:row.title,type:row.resource_type,sourceUrl:row.source_url??"",storagePath:row.storage_path??"",mimeType:row.mime_type??"",fileSize:row.file_size_bytes,sortOrder:row.sort_order,embedProvider:row.embed_provider??""}));
...
export async function addLinkedResource(lesson:Lesson,draft:{title:string;url:string;type:"LINK"|"YOUTUBE"|"EMBED";provider?:string}){const next=lesson.resources.reduce((max,item)=>Math.max(max,item.sortOrder),0)+1;const{error}=await getSupabaseBrowserClient().from("lesson_resources").insert({organization_id:lesson.organizationId,lesson_id:lesson.id,title:draft.title.trim(),resource_type:draft.type==="EMBED"?"LINK":draft.type,source_url:draft.url.trim(),embed_provider:draft.type==="EMBED"?draft.provider:null,sort_order:next});if(error)throw error}
