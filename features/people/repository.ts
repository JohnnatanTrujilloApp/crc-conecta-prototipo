...
type PersonRow={id:string;crc_code?:string;first_name:string;last_name:string;preferred_name:string|null;email:string|null;phone:string;site_id:string;person_status:Person["status"];first_visit_date:string|null;birth_date:string|null;baptized?:boolean;sites:{name:string}|{name:string}[]|null};
...
export async function loadPeopleData(){
 ...
 const [sitesResult,peopleResult]=await Promise.all([
  ...
  client.from("people").select("id,crc_code,first_name,last_name,preferred_name,email,phone,site_id,person_status,first_visit_date,birth_date,baptized,sites(name)").is("archived_at",null).order("created_at",{ascending:false}),
 ]);
...
export async function createPerson(draft:PersonDraft,site:SiteOption):Promise<Person>{
 const {data,error}=await getSupabaseBrowserClient().from("people").insert({organization_id:site.organizationId,site_id:site.id,first_name:draft.firstName.trim(),last_name:draft.lastName.trim(),email:draft.email?.trim()||null,phone:draft.phone.trim(),person_status:draft.status,birth_date:draft.birthDate,first_visit_date:new Date().toISOString().slice(0,10)}).select("id,crc_code,first_name,last_name,preferred_name,email,phone,site_id,person_status,first_visit_date,birth_date,baptized").single();
...
export async function updatePerson(id:string,draft:PersonDraft,site:SiteOption):Promise<Person>{
 const {data,error}=await getSupabaseBrowserClient().from("people").update({organization_id:site.organizationId,site_id:site.id,first_name:draft.firstName.trim(),last_name:draft.lastName.trim(),email:draft.email?.trim()||null,phone:draft.phone.trim(),person_status:draft.status,birth_date:draft.birthDate}).eq("id",id).select("id,crc_code,first_name,last_name,preferred_name,email,phone,site_id,person_status,first_visit_date,birth_date,baptized").single();
 if(error)throw error;return mapPerson(data as PersonRow,site);
}
