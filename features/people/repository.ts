...
export async function createPerson(draft:PersonDraft,site:SiteOption):Promise<Person>{
 const {data,error}=await getSupabaseBrowserClient().from("people").insert({organization_id:site.organizationId,site_id:site.id,document_type:draft.documentType||null,document_number:draft.documentNumber?.trim().toUpperCase()||null,first_name:draft.firstName.trim(),last_name:draft.lastName.trim(),email:draft.email?.trim()||null,phone:draft.phone.trim(),person_status:draft.status,birth_date:draft.birthDate,first_visit_date:new Date().toISOString().slice(0,10)}).select("id,crc_code,document_type,document_number,first_name,last_name,preferred_name,email,phone,site_id,person_status,first_visit_date,birth_date,baptized").single();
...
export async function updatePerson(id:string,draft:PersonDraft,site:SiteOption):Promise<Person>{
 const {data,error}=await getSupabaseBrowserClient().from("people").update({organization_id:site.organizationId,site_id:site.id,document_type:draft.documentType||null,document_number:draft.documentNumber?.trim().toUpperCase()||null,first_name:draft.firstName.trim(),last_name:draft.lastName.trim(),email:draft.email?.trim()||null,phone:draft.phone.trim(),person_status:draft.status,birth_date:draft.birthDate}).eq("id",id).select("id,crc_code,document_type,document_number,first_name,last_name,preferred_name,email,phone,site_id,person_status,first_visit_date,birth_date,baptized").single();
 if(error)throw error;return mapPerson(data as PersonRow,site);
}
