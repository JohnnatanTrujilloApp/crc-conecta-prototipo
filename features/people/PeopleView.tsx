...
export function PeopleView() {
  ...
  return <div className="content people-content">
    ...
    {selectedPerson&&<div className="profile-layer"><button className="profile-scrim" onClick={()=>setSelectedPerson(null)} aria-label="Cerrar perfil"/><aside className="person-profile"><div className="profile-head"><div className="profile-avatar">{selectedPerson.firstName[0]}{selectedPerson.lastName[0]}</div><div><span className="eyebrow">PERFIL DE PERSONA</span><h2>{selectedPerson.firstName} {selectedPerson.lastName}</h2><p>{selectedPerson.siteName} · {statusLabels[selectedPerson.status]}</p></div><button onClick={()=>setSelectedPerson(null)} aria-label="Cerrar">×</button></div><div className="profile-code"><span>Código CRC</span><strong>CRC-00010{selectedPerson.id.slice(-1)}</strong><small>Identificador ficticio</small></div><section className="profile-section"><h3>Información de contacto</h3><div className="profile-facts"><p><span>Celular</span><strong>{selectedPerson.phone}</strong></p><p><span>Correo</span><strong>{selectedPerson.email??"Sin correo"}</strong></p><p><span>Primera visita</span><strong>{selectedPerson.firstVisitDate}</strong></p><p><span>Bautismo</span><strong>{selectedPerson.status==="MEMBER"?"Sí":"Pendiente"}</strong></p></div></section><section className="profile-section"><h3>Familia</h3><button className="profile-link"><span>⌂</span><div><strong>Familia {selectedPerson.lastName} Gómez</strong><small>3 integrantes vinculados</small></div><b>→</b></button></section><section className="profile-section"><h3>Proceso y servicio</h3><div className="profile-progress"><div><span>Discipulado CRC</span><strong>80%</strong></div><i><b style={{width:"80%"}}/></i></div><div className="profile-tags"><span>Ujieres · Servidor</span><span>En formación</span></div></section><div className="profile-actions"><button className="secondary-button">Editar perfil</button><button className="primary-button">Ver seguimiento</button></div></aside></div>}
  </div>;
}
