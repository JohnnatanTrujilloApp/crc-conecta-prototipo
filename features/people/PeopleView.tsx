...
const seedPeople: Person[] = [
  { id: "demo-1", firstName: "María", lastName: "Rodríguez", phone: "300 555 0101", email: "maria@example.com", siteId: "nemocon-demo", siteName: "CRC Nemocón Demo", status: "MEMBER", firstVisitDate: "2026-02-08", birthDate: "1990-04-12" },
  { id: "demo-2", firstName: "Pedro", lastName: "Gómez", phone: "301 555 0102", siteId: "nemocon-demo", siteName: "CRC Nemocón Demo", status: "CONGREGANT", firstVisitDate: "2026-04-19", birthDate: "1987-09-03" },
  { id: "demo-3", firstName: "Ana", lastName: "Martínez", phone: "302 555 0103", email: "ana@example.com", siteId: "central-demo", siteName: "CRC Central Demo", status: "VISITOR", firstVisitDate: "2026-08-16", birthDate: "2001-01-20" },
  { id: "demo-4", firstName: "Carlos", lastName: "Pérez", phone: "303 555 0104", siteId: "nemocon-demo", siteName: "CRC Nemocón Demo", status: "LEADER", firstVisitDate: "2025-11-02", birthDate: "1978-06-17" },
];
...
export function PeopleView() {
  ...
  const savePerson = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setErrors({});
    setFormOpen(false);
    setNotice(configured&&session?`Persona ${editingId?"actualizada":"guardada"} en Supabase con el alcance de la sede.`:`Persona ${editingId?"actualizada":"agregada"} en el modo demostración.`);setEditingId(null);}catch(error){const duplicate=typeof error==="object"&&error!==null&&"code" in error&&(error as {code?:string}).code==="23505";setLoadError(duplicate?"Ya existe una persona con ese tipo y número de documento.":"No fue posible guardar. Confirma que tu rol tenga permisos sobre esta sede.")}
  };

  const editPerson=(person:Person)=>{setEditingId(person.id);setDraft({firstName:person.firstName,lastName:person.lastName,email:person.email??"",phone:person.phone,birthDate:person.birthDate,documentType:person.documentType,documentNumber:person.documentNumber??"",siteId:person.siteId,status:person.status});setErrors({});setNotice("");setSelectedPerson(null);setFormOpen(true)};
