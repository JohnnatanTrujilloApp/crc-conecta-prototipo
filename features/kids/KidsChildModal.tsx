...
export function KidsChildModal({attendanceEventId,close,saved}:{attendanceEventId?:string;close:()=>void;saved:()=>Promise<void>}){
 ...
 const needsRelationship=hasGuardian&&!existingRelationship;
 ...
 const submit=async(e:React.FormEvent)=>{e.preventDefault();if(birthError){setError(kidsErrorMessage[birthError]);return}if(linkLoading||linkError)return;if(needsRelationship&&!draft.guardianRelationship){setError(kidsErrorMessage.KIDS_GUARDIAN_RELATIONSHIP_REQUIRED);return}setBusy(true);try{await saveKidsChild(draft);await saved()}catch(err){console.error("Kids registration",err);setError(explainKidsError(err))}finally{setBusy(false)}};
