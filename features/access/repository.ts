...
export async function loadPortalContext(timeoutMs=12000){
 ...
 const request=getSupabaseBrowserClient().rpc("get_my_portal_context").abortSignal(controller.signal).then(({data,error})=>{if(error)throw error;if(!data)throw new Error("PORTAL_CONTEXT_EMPTY");return data as PortalContext});
