...
export async function loadAccessRequests(){const{data,error}=await getSupabaseBrowserClient().rpc("list_authorized_access_requests");if(error)throw error;const rows=(Array.isArray(data)?data:[]) as AccessRequestRow[];return rows.filter(row=>row.status==="PENDING_APPROVAL").map(row=>({id:row.id,status:row.status,createdAt:row.created_at,possibleDuplicate:row.possible_duplicate,observations:row.observations??"",person:row.person,site:row.site}))}
export async function reviewAccessRequest(id:string,decision:"APPROVE"|"REJECT",observations:string){const{error}=await getSupabaseBrowserClient().rpc("review_access_request",{target_request_id:id,decision,given_observations:observations||null});if(error)throw error}
