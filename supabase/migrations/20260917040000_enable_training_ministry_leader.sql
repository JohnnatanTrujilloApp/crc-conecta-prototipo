-- El Líder de Formación administra el catálogo formativo sin abrir Personas ni Reportes.
...
$$;
revoke all on function public.current_user_is_training_ministry_leader(uuid,uuid) from public;
grant execute on function public.current_user_is_training_ministry_leader(uuid,uuid) to authenticated;
...
returns boolean language sql stable security definer set search_path='' as $$
 select exists(select 1 from public.user_accounts ua join public.user_roles ur on ur.user_account_id=ua.id join public.roles r on r.id=ur.role_id join public.ministries m on m.organization_id=ur.organization_id and m.leader_person_id=ua.person_id join public.person_ministries pm on pm.ministry_id=m.id and pm.person_id=ua.person_id where ua.id=auth.uid() and ua.access_status='ACTIVE' and ur.active and ur.starts_at<=now() and(ur.ends_at is null or ur.ends_at>now()) and r.code='MINISTRY_LEADER' and m.active and pm.active and lower(trim(m.name)) in('alabanza','alabanza y adoración','ministerio de alabanza','ministerio de alabanza y adoración','damas','ministerio de damas','caballeros','ministerio de caballeros','parejas','ministerio de parejas','kids','crc kids','ministerio kids','ministerio de niños','intercesión','intercesion','ministerio de intercesión','ministerio de intercesion','ujieres','ministerio de ujieres','finanzas','ministerio de finanzas','formación','formacion','ministerio de formación','ministerio de formacion') and(target_organization_id is null or ur.organization_id=target_organization_id) and(target_site_id is null or m.site_id=target_site_id))
 and not exists(select 1 from public.user_roles ur join public.roles r on r.id=ur.role_id where ur.user_account_id=auth.uid() and ur.active and ur.starts_at<=now() and(ur.ends_at is null or ur.ends_at>now()) and r.code in('SUPER_ADMIN','NATIONAL_PASTOR','SITE_PASTOR'));
$$;
...
returns boolean language sql stable security definer set search_path='' as $$
 select not(required_permission in('people.read','people.create','people.update','people.archive','families.read','families.create','families.update','training.manage','followups.read','followups.manage','reports.read','reports.export') and public.current_user_is_restricted_discipleship_coordinator(target_organization_id,target_site_id))
 and not(required_permission in('ministries.read','ministries.manage') and not public.current_user_has_pastoral_ministry_access(target_organization_id,target_site_id))
 and not(required_permission in('people.read','people.create','people.update','people.archive','reports.read','reports.export') and public.current_user_is_restricted_specialized_ministry_leader(target_organization_id,target_site_id))
 ...
 or(required_permission='training.manage' and public.current_user_is_training_ministry_leader(target_organization_id,target_site_id)));
$$;

create or replace function public.current_user_has_any_permission(required_permission text)
returns boolean language sql stable security definer set search_path='' as $$
 select not(required_permission in('people.read','people.create','people.update','people.archive','families.read','families.create','families.update','training.manage','followups.read','followups.manage','reports.read','reports.export') and public.current_user_is_restricted_discipleship_coordinator(null,null))
 and not(required_permission in('ministries.read','ministries.manage') and not public.current_user_has_pastoral_ministry_access(null,null))
 and not(required_permission in('people.read','people.create','people.update','people.archive','reports.read','reports.export') and public.current_user_is_restricted_specialized_ministry_leader(null,null))
 ...
 or(required_permission='training.manage' and public.current_user_is_training_ministry_leader(null,null)));
$$;

create or replace function public.current_user_has_org_permission(required_permission text,target_organization_id uuid)
returns boolean language sql stable security definer set search_path='' as $$
 select not(required_permission in('people.read','people.create','people.update','people.archive','families.read','families.create','families.update','training.manage','followups.read','followups.manage','reports.read','reports.export') and public.current_user_is_restricted_discipleship_coordinator(target_organization_id,null))
 and not(required_permission in('ministries.read','ministries.manage') and not public.current_user_has_pastoral_ministry_access(target_organization_id,null))
 and not(required_permission in('people.read','people.create','people.update','people.archive','reports.read','reports.export') and public.current_user_is_restricted_specialized_ministry_leader(target_organization_id,null))
 and(exists(select 1 from public.user_accounts ua join public.user_roles ur on ur.user_account_id=ua.id join public.role_permissions rp on rp.role_id=ur.role_id join public.permissions p on p.id=rp.permission_id where ua.id=auth.uid() and ua.access_status='ACTIVE' and ur.active and ur.starts_at<=now() and(ur.ends_at is null or ur.ends_at>now()) and ur.organization_id=target_organization_id and p.code=required_permission)
 or(required_permission='training.manage' and public.current_user_is_training_ministry_leader(target_organization_id,null)));
$$;
