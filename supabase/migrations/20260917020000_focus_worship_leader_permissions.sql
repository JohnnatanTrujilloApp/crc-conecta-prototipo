-- El Líder de Alabanzas trabaja desde su módulo especializado; no requiere Personas ni Reportes.
...
returns boolean language sql stable security definer set search_path='' as $$
 select exists(select 1 from public.user_accounts ua join public.user_roles ur on ur.user_account_id=ua.id join public.roles r on r.id=ur.role_id join public.ministries m on m.organization_id=ur.organization_id and m.leader_person_id=ua.person_id join public.person_ministries pm on pm.ministry_id=m.id and pm.person_id=ua.person_id where ua.id=auth.uid() and ua.access_status='ACTIVE' and ur.active and ur.starts_at<=now() and(ur.ends_at is null or ur.ends_at>now()) and r.code='MINISTRY_LEADER' and m.active and pm.active and lower(trim(m.name)) in('alabanza','alabanza y adoración','ministerio de alabanza','ministerio de alabanza y adoración') and(target_organization_id is null or ur.organization_id=target_organization_id) and(target_site_id is null or m.site_id=target_site_id))
...
$$;
revoke all on function public.current_user_is_restricted_worship_leader(uuid,uuid) from public;
grant execute on function public.current_user_is_restricted_worship_leader(uuid,uuid) to authenticated;
...
declare ua record; person record; effective_permissions jsonb; roles_json jsonb; begin
 ...
 and not(permission.code in('people.read','people.create','people.update','people.archive','reports.read','reports.export') and public.current_user_is_restricted_worship_leader(null,null))
