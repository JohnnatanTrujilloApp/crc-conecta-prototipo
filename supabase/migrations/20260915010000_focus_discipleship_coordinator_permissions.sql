-- El líder de Discipulado trabaja desde su módulo especializado y su Excel.
-- Los permisos heredados de otros roles operativos no deben abrir Formación,
-- Seguimiento ni Reportes, salvo que posteriormente reciba un rol superior.

create or replace function public.current_user_is_restricted_discipleship_coordinator(
  ...
  select
    exists(
      ...
      where ur.user_account_id=auth.uid()
        ...
        and r.code='DISCIPLESHIP_COORDINATOR'
        and(target_organization_id is null or ur.organization_id=target_organization_id)
        and(target_site_id is null or ur.scope_type='ORGANIZATION' or ur.site_id=target_site_id)
    )
    and not exists(
      select 1
      from public.user_roles ur
      ...
      where ur.user_account_id=auth.uid()
        and ur.active and ur.starts_at<=now() and(ur.ends_at is null or ur.ends_at>now())
        and r.code in('SUPER_ADMIN','NATIONAL_PASTOR','SITE_PASTOR','SITE_ADMIN')
        and(target_organization_id is null or ur.organization_id=target_organization_id)
        and(target_site_id is null or ur.scope_type='ORGANIZATION' or ur.site_id=target_site_id)
    );
$$;

revoke all on function public.current_user_is_restricted_discipleship_coordinator(uuid,uuid) from public;
grant execute on function public.current_user_is_restricted_discipleship_coordinator(uuid,uuid) to authenticated;
...
comment on function public.current_user_is_restricted_discipleship_coordinator(uuid,uuid)
is 'Limita áreas administrativas redundantes para coordinadores de Discipulado, salvo que tengan un rol administrativo superior.';
