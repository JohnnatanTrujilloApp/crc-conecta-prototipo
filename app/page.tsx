...
function Application(){
  ...
  if(portalLoading||!portal)return <main className="login-page"><div className="login-card"><strong>{portalError||"Validando acceso autorizado…"}</strong><p>{portalError?"La consulta no respondió, pero tu sesión permanece protegida.":"Puedes esperar o utilizar una de estas opciones."}</p><div className="login-actions"><button className="primary-button" disabled={portalLoading} onClick={()=>setPortalAttempt(attempt=>attempt+1)}>{portalLoading?"Validando…":"Reintentar"}</button><button className="secondary-button" onClick={()=>void signOut()}>Cerrar sesión</button><button className="secondary-button" onClick={()=>openView("public","/?public=1")}>Portal público</button></div></div></main>;
