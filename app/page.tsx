...
type View = "dashboard" | "people" | "families" | "ministries" | "worship" | "kids" | "attendance" | "discipleship" | "classroom" | "training" | "followups" | "reports" | "content" | "access" | "public" | "register";
...
function Application(){
  ...
  useEffect(()=>{if(!configured||!accessToken){setPortal(null);setPortalLoading(false);return}let active=true;setPortalLoading(true);setPortalError("");const deadline=window.setTimeout(()=>{if(!active)return;active=false;setPortal(null);setPortalLoading(false);setPortalError("La validación tardó más de 12 segundos. Puedes reintentar o cerrar la sesión.")},12500);void loadPortalContext(accessToken,10000).then(next=>{if(!active)return;setPortal(next);setPortalError("")}).catch(error=>{if(!active)return;setPortal(null);setPortalError(error instanceof Error&&error.message==="PORTAL_CONTEXT_TIMEOUT"?"La validación tardó más de 12 segundos. Puedes reintentar o cerrar la sesión.":"No fue posible validar el acceso en este momento. Vuelve a intentarlo.")}).finally(()=>{if(active){active=false;window.clearTimeout(deadline);setPortalLoading(false)}});return()=>{active=false;window.clearTimeout(deadline)}},[configured,accessToken,portalAttempt]);
  ...
  if(!configured)return <main className="login-page"><div className="login-card"><strong>No fue posible iniciar la conexión segura.</strong><p>Recarga la página. Si continúa, informa este mensaje al administrador.</p><div className="login-actions"><button className="primary-button" onClick={()=>window.location.reload()}>Recargar</button><button className="secondary-button" onClick={()=>openView("public","/?public=1")}>Portal público</button></div></div></main>;
  ...
  const effective=new Set(portal.permissions);const isAdministrative=["people.read","families.read","attendance.register","groups.manage","worship.view_team","kids.view","training.manage","followups.read","ministries.read","reports.read","content.manage","access_requests.review","roles.manage","events.create","events.manage_ministry"].some(permission=>effective.has(permission));
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${menuOpen?"sidebar-open":""}`}>
      ...
      <div className="sidebar-utilities" aria-label="Utilidades">
        ...
        <div className="sidebar-help"><span>?</span><span><strong>Centro de ayuda</strong><small>Guías y soporte</small></span></div>
